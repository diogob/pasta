...
main = hspec $ do
  ...
  describe "select" $ do
    it "should build select command using fromClause setter" $
      showt (select & fromClause .~ ["table1", "table2"]) `shouldBe` "SELECT * FROM \"table1\" \"table1\", \"table2\" \"table2\""
    it "should build select command using fromClause and where setters" $
      showt (select & fromClause .~ ["table1"] & setWhere t) `shouldBe` "SELECT * FROM \"table1\" \"table1\" WHERE true"
